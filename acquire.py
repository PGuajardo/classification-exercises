...
def get_telco_data():
    ...
    if os.path.isfile(filename):
        ...
        teleco_churn = pd.read_sql('SELECT * FROM customers JOIN contract_types using(contract_type_id) JOIN internet_service_types using(internet_service_type_id) JOIN payment_types using(payment_type_id)', 
        get_connection('teleco_churn'))
